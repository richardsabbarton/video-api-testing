...
getSessionCredentials(urlApiKey, roomName).then((json) => {
  timestamps.getSessionCredentialsTook =
    performance.now() - timestamps.getSessionCredentials;
  apiKey = json.apiKey;
...
function initializeSession() {
  ...
  session.on("connectionCreated", (event) => {
    console.log(event.type, event.connection.id);
  });
  ...
  timestamps.createPublisherTook =
    performance.now() - timestamps.createPublisher;

  // Connect to the session
  timestamps.sessionConnect = performance.now();
  session.connect(token, (error) => {
    timestamps.sessionConnectTook =
      performance.now() - timestamps.sessionConnect;
